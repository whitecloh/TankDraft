...
namespace TankDraft.AdmissionHttpChecks;
...
internal sealed class AuthoredContent
{
    ...
    public static (AuthoredContent Content, string Version) Load()
    {
        ...
        if (content.SchemaVersion != 1 || content.MatchRules is null || content.BattleRules is null ||
            content.BattleRules.AllowDebugCommands || !MatchService.IsSupportedDeck(content.Deck, content.MatchUnits))
        ...
        if (content.OrderId is not "" and not "order.reinforce_armor")
            throw new InvalidDataException("Order is not supported by the current match domain.");
